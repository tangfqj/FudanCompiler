...
void munchCjump(T_stm s){
  ...
  if(left == T_int && right == T_int){
    ...
    if(!s0 && !s1){
      sprintf(ir, "%%`d0 = icmp %s i64 %d, %d", op, s->u.CJUMP.left->u.CONST.i, s->u.CJUMP.right->u.CONST.i);
      ...
      emit(AS_Oper(ir, Temp_TempList(cond, NULL), Temp_TempList(s0, Temp_TempList(s1, NULL)), NULL));
    }
  } else{ // T_float
    switch(s->u.CJUMP.op){
      case T_eq:
    ...
    if(!s0 && !s1){
      sprintf(ir, "%%`d0 = fcmp %s i64 %f, %f", op, s->u.CJUMP.left->u.CONST.f, s->u.CJUMP.right->u.CONST.f);
      emit(AS_Oper(ir, Temp_TempList(cond, NULL), NULL, NULL));
    } else if(!s0){
  ...
  emit(AS_Oper(brch, NULL, Temp_TempList(cond, NULL), AS_Targets(LL(s->u.CJUMP.t, LL(s->u.CJUMP.f, NULL)))));
}
...
Temp_temp munchBinop(Temp_temp ret, T_exp e){
  ...
  if(left == T_int && right == T_int){
    ...
    if(!s0 && !s1){
      sprintf(ir, "%%`d0 = %s i64 %d, %d", op,  e->u.BINOP.left->u.CONST.i, e->u.BINOP.right->u.CONST.i);
      emit(AS_Oper(ir, Temp_TempList(ret, NULL), NULL, NULL));
    } else if(!s0){
      ...
      emit(AS_Oper(ir, Temp_TempList(ret, NULL), Temp_TempList(s0, Temp_TempList(s1, NULL)), NULL));
    }
  } else{ // T_float
    if(ret == NULL){
      ret = Temp_newtemp(T_float);
    ...
    if(!s0 && !s1){
      sprintf(ir, "%%`d0 = %s double %f, %f", op,  e->u.BINOP.left->u.CONST.f, e->u.BINOP.right->u.CONST.f);
      emit(AS_Oper(ir, Temp_TempList(ret, NULL), NULL, NULL));
    } else if(!s0){
...
void munchExtCall(Temp_temp ret, T_exp e) {
  ...
  if (strcmp(func, "putint") == 0) {
    ...
    if(e->u.ExtCALL.args->head->kind == T_CONST){
      sprintf(ir, "%%`d0 = call i64* @malloc(i64 %d)", e->u.ExtCALL.args->head->u.CONST.i);
